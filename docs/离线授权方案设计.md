...
2.  **验证**：服务端收到`.unbind`文件后，首先解密文件，然后用**服务端主公钥**验证`signed_license.signature`，确保授权本身是合法的。然后，根据授权信息从数据库查出对应的**一次性解绑公钥**，用它来验证`unbind_proof`。双重验证确保解绑请求来自一个合法的、且真正执行了解绑操作的客户端。
