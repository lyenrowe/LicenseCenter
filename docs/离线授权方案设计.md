...
**获取授权码列表**
```http
GET /api/v1/admin/authorizations?page=1&limit=20&search=张三&status=active
```

**更新授权码**
```http
PUT /api/v1/admin/authorizations/{id}
Content-Type: application/json
...
**强制解绑设备**
```http
POST /api/v1/admin/licenses/{license_id}/force-unbind
Content-Type: application/json
...
**获取客户详情**
```http
GET /api/v1/admin/authorizations/{id}/details
```
...
**获取操作日志**
```http
GET /api/v1/admin/logs?page=1&limit=50&action=create_authorization
```
